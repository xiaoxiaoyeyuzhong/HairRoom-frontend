...
// 数据结构映射函数
const mapTags = (tagList) => {
  return tagList.map(tag => ({
    text: tag.parentTagName,  // 将 parentTagName 映射为 text
    children: tag.childrenTagList ? mapChildrenTags(tag.childrenTagList) : undefined  // 递归映射子标签
  }));
};
// 获取标签列表
const fetchTags = async () => {
  try {
    const response = await myAxios.post('/tag/list');
    // 将后端传递的数据映射为前端所需的格式
    originTagList = mapTags(response.data);
    // tagList = ref(originTagList);
    tagList.value = originTagList;
    showSuccessToast('获取标签列表成功');
  } catch (error) {
    showFailToast('获取标签列表失败' + (res.description ? `，${res.description}` : ''));
  }
};
